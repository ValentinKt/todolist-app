<template>
	<div class="page-container">
		<div class="content-wrapper">
			...
			<TodoHeader :completed-count="completedCount" :total-count="filteredTodos.length"
				:progress-percentage="progressPercentage" @toggle-sort="toggleSortOrder" />

			<!-- Main todo interface -->
			<div class="todo-interface">
				<!-- Todo input form -->
				<TodoForm @add-todo="addTodo" class="mb-6" />
				...
				<!-- Empty state -->
				<EmptyState v-else-if="!filteredTodos.length" @add-todo="focusInput" />

				<!-- Todo list with transition animations -->
				<TransitionGroup v-else name="list" tag="ul" class="todo-list">
					<TodoItem v-for="(todo, index) in sortedTodos" :key="todo.id" :todo="todo" :index="index"
						@toggle="toggleTodoCompletion(todo)" @delete="deleteTodo(todo.id)" @reorder="handleReorder"
						class="todo-item" />
				</TransitionGroup>

				<!-- Filter controls -->
				<TodoFilters v-if="hasTodos" v-model:filter="currentFilter" v-model:sort="sortOrder" class="mt-6" />
			</div>
		</div>

		<!-- Celebration effects -->
		<ConfettiEffect v-if="showCelebration" />
		<AchievementToast v-if="achievement" :achievement="achievement" @close="achievement = null" />
	</div>
</template>

<script setup lang="ts">
import type { Todo, TodoFilter, TodoAchievement } from '~/types/types'
import { useTodosStore } from '~/composables/state/useTodosStore'
...
const addTodo = async (text: string) => {
	await todoStore.addTodo(text as Partial<Todo>)
